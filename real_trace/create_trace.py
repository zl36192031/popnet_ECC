...
def change_real_trace(res):
    result = [[0 for x1 in range(10)] for x2 in range(10)]
    for i in res :
        list1 = [x for x in i.split()]
        result[int(list1[2])][int(list1[3])] += 1
    for i in range(10):
        for j in range(10):
...
if __name__ == '__main__':
    res = read_bench('flow_banres_xy_org.log')
    change_real_trace(res)
# exec
